...
export class PolicyEvaluator implements PolicyEvaluatorContract {
  ...
  private evaluateMerchant(
    ...
    const domain = canonicalizeDomain(context.checkout.merchant.domain);
    ...
    const vendorId = context.checkout.merchant.vendorId;
  ...
  private evaluateVelocity(
    ...
    const currentDomain = canonicalizeDomain(context.checkout.merchant.domain);
  ...
  private normalizeToMandateCurrency(
    ...
    if (!getMinorDigits(sourceCurrency) || !getMinorDigits(mandateCurrency)) {
      // JPY has zero minor digits, so explicit undefined checks are required.
      if (
  ...
  private evaluateSpendLimits(
    ...
    if (
      normalizeCurrency(context.spend.committedDailySpend.currency) !==
        spendCurrency ||
...
function convertMoneyCeiling(
  ...
  const scaledNumerator =
    money.minorUnits * numerator * pow10(targetDigits);
