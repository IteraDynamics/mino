...
const paymentAmountMinor = "25000";
...
integration("production administrative transaction and approval HTTP surface", () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `insert into "PaymentOutcome" (
         ...
         'merchant-1', 'shop.example.com', 'checkout-1', $6::bigint, 'USD', 'UNKNOWN', 503,
         $7::jsonb, $8::jsonb, 'MERCHANT_TRANSPORT_ERROR', $9, $10, 2, $11,
         'operations-lease-secret', $12, $9, $10
       )`,
      [
        paymentId,
        organizationId,
  ...
  it("exposes safe operational reads and routes votes through real JWT/RBAC without payment override authority", async () => {
      ...
      adminJwtIssuers: [
        {
          issuer,
          audience,
          verificationKeys: new Map([["operations-rsa-1", pemPublic(jwtKeys.publicKey)]]),
        },
      ],
    });
    ...
    try {
      ...
      expect(replay.json()).toMatchObject({ outcome: "REPLAYED", changed: false });
